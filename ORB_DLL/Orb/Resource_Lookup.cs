...
namespace ORB_DLL.Orb
{
	public class Resource_Lookup
	{
		...
		{
			dataFileName = @"C:\Users\lhann\OneDrive\Documents\Software\ORB_DATABASE.xlsx";
		...
		public DataTable GetCountiesByState(string state)
		{
			...
			cmdGetCountiesByState.CommandText = string.Concat("Select st, county From [orb$] WHERE [st]='", state, "'");
		...
		public Online_Resource GetResources(string state, string county, string tax_auth)
		{
			...
			cmdGetResource.CommandText = string.Concat("Select * From [orb$] where st = '", state, "' and county = '", county, "'");
